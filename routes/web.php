...
use App\Http\Controllers\ConfigurationRuleController;
...
Route::middleware(['auth', 'check.tenant_id', 'verified'])->group(function () {
    ...
    Route::get('configuraciones/{applicationId}', [ApplicationController::class, 'getConfiguraciones'])->name('configuraciones.aplications');
    ...
    Route::resource('notificationSetting', NotificationSetting::class);
    ...
    Route::post('/store/configuration/rules/button', [ConfigurationRuleController::class, 'storeButtonConfiguration'])->name('store.configuration.rules.button');
    Route::delete('/configuration/rules/{id}', [ConfigurationRuleController::class, 'destroy'])->name('configuration.rules.destroy');
    Route::post('/store/configuration/rules/gps', [ConfigurationRuleController::class, 'storeGps'])->name('store.configuration.rules.gps');
    Route::post('/store/configuration/rules/distance', [ConfigurationRuleController::class, 'storeDistance'])->name('store.configuration.rules.distance');

    Route::get('{id}/dispositivo/lecturas', [DispositivoController::class, 'lecturas'])->name('dispositivo.lecturas');
    Route::get('dispositivos/consultar/lecturas', [DispositivoController::class, 'getLecturasNoconfiguradas'])->name('dispositivos.consultar.lecturas');
});
